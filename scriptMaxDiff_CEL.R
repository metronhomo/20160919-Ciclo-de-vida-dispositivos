...
base <- base[base$P10_Celular==T,]


#Obtenemos el subconjunto de la base con las evaluaciones de los atributos
z<-base %>% dplyr::select(contains('CELULARES'))
head(z)

###convierto a caracter,quito espacios,convierto otra vez a caracter
for(j in 1:length(z)){
    z[,j] <- as.character(z[,j])
    z[,j] <- gsub(" ","",z[,j])
    z[,j] <- as.factor(z[,j])
}
...
for(j in c(9:10)){#bloque5 ejecutar a partir de AQUÍ
    for(i in 1:nrow(z)){
        if(z[i,j]==1)z[i,j]=18
        if(z[i,j]==2)z[i,j]=12
        if(z[i,j]==4)z[i,j]=8
...
# Creamos un dataframe con NA's de tamaño 1116*13 renglones, donde 1116 es el no. de 

...
#######Agrupacion de preguntas para P10
##Uso de dispositivos
##CELULAR

variablesR <- nombresR(datos,"P10_1_CELULAR_USO")
objetivos <- levels(datos$P10_1_CELULAR_USO1)
listos <- c("Yo","EsposoEsposa","PapaMama","HijosHijas","Otro","NsNc")
for(i in 1:length(objetivos)){
    # i <- 1
    objetivito <- objetivos[i]
    for(t in 1:length(variablesR)){
        # t <- 1
        logicc <- datos[,variablesR[t]]==objetivito
        logicc[is.na(logicc)] <- F
        datos[logicc,paste("Rr_P10_1_CELULAR_",listos[i],sep="")] <- T
    }
}
...
variablesR <- nombresR(datos,"P10_1_TABLET_USO")
objetivos <- levels(datos$P10_1_TABLET_USO1)
listos <- c("Yo","EsposoEsposa","PapaMama","HijosHijas","Otro","NsNc")
for(i in 1:length(objetivos)){
    # i <- 1
    objetivito <- objetivos[i]
    for(t in 1:length(variablesR)){
        ...
        logicc <- datos[,variablesR[t]]==objetivito
        logicc[is.na(logicc)] <- F
        datos[logicc,paste("Rr_P10_1_TABLET_",listos[i],sep="")] <- T
    }
}

#sample(letters,100,T)=="a"

##COMPU

variablesR <- nombresR(datos,"P10_1_COMPU_USO")
objetivos <- levels(datos$P10_1_COMPU_USO1)
listos <- c("Yo","EsposoEsposa","PapaMama","HijosHijas","Otro","NsNc")
for(i in 1:length(objetivos)){
    # i <- 1
    objetivito <- objetivos[i]
    for(t in 1:length(variablesR)){
        ...
        logicc <- datos[,variablesR[t]]==objetivito
        logicc[is.na(logicc)] <- F
        datos[logicc,paste("Rr_P10_1_COMPU_",listos[i],sep="")] <- T
    }
}
...
#BANNER2: USO EXCLUSIVO CELULAR
datos <- datos[datos$P15_Celular==T,]

#BANNER2: USO EXCLUSIVO TABLET
datos <- datos[datos$P15_Tablet___Ipad==T,]

#BANNER2: USO EXCLUSIVO COMPU_LAPTOP
datos <- datos[datos$P15_Computadora__Lap_Top==T,]

#BANNER2: USO COMPARTIDO TABLET
datos <- datos[datos$P15_1_Tablet___Ipad==T,]

#BANNER2: USO COMPARTIDO COMPU_LAPTOP
datos <- datos[datos$P15_1_Computadora__Lap_Top==T,]

####################################################


bandera<- c("Total","F1Genero","NSE","Edad_Rango","Plaza","EdocivilHIjos")
...
exportator(resultVarBanner,"resultVarBanner2_CompartidoCOMPU.csv")
...
resultBanner2 <- list(
    ...
    P10_quienUsaCEL = frecuentator(fTtabla = datos[datos$P10_Celular==T,],fTvariables = nombresR(datos,"Rr_P10_1_CELULAR"),fTlevels = F,fbanner = bandera),
    P10_quienUsaTABLET = frecuentator(fTtabla = datos[datos$P10_Tablet___Ipad==T,],fTvariables = nombresR(datos,"Rr_P10_1_TABLET"),fTlevels = F,fbanner = bandera),
    P10_quienUsaCOMPU = frecuentator(fTtabla = datos[datos$P10_Computadora___Lap_Top==T,],fTvariables = nombresR(datos,"Rr_P10_1_COMPU"),fTlevels = F,fbanner = bandera),
...
resultBanner2 <- list(
    estadoCivilA4 = frecuentator(fTtabla = datos,fTvariables = 'A4',fTlevels = T,fbanner = bandera),
    
    tiempoCasadosA5 = frecuentator(fTtabla = datos,fTvariables = 'TiempoJuntos',fTlevels = T,fbanner = bandera),
    
    tieneHijosA6 = frecuentator(fTtabla = datos,fTvariables = 'tieneHijos',fTlevels = T,fbanner = bandera),
    
    cuantosHijosA7 = frecuentator(fTtabla = datos[datos$tieneHijos=='Sí',],fTvariables = 'numHijos',fTlevels = T,fbanner = bandera),
    
    hijosVivenConUstedA8 = frecuentator(fTtabla = datos[datos$tieneHijos=='Sí',],fTvariables = 'hijosSinEmancipar',fTlevels = T,fbanner = bandera),
    
    
    edadHijos = frecuentator(fTtabla = datos[datos$tieneHijos=='Sí',],fTvariables = hijitosEdad,
                             fTlevels = T,fbanner = bandera),
    
    composicionFamiliaA11 = frecuentator(fTtabla = datos,fTvariables = 'A11',fTlevels = T,fbanner = bandera)
    
    
)

exportator(resultBanner2,"resultadosBanner2_A4_A11.csv")


resultBanner2 <- list(
    estadoCivilA4 = frecuentator(fTtabla = datos[datos$P10_Celular==T,],fTvariables = 'A4',fTlevels = T,fbanner = bandera),
    
    tiempoCasadosA5 = frecuentator(fTtabla = datos[datos$P10_Celular==T,],fTvariables = 'TiempoJuntos',fTlevels = T,fbanner = bandera),
    
    tieneHijosA6 = frecuentator(fTtabla = datos[datos$P10_Celular==T,],fTvariables = 'tieneHijos',fTlevels = T,fbanner = bandera),
